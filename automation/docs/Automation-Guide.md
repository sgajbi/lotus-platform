...
| Bounded canonical recheck | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -ValidationProfile client-demo` | Recheck an already-running governed stack without seeding DPM state; records the disabled seed and excluded presentation-backed Idea capacity probe, and does not certify full capacity or release readiness |
...
| Canonical front-office screenshot pack | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -BringUp` | Run full startup and validation before writing the default demo screenshots, `SHOT-INDEX.md`, and structured evidence; pass a caller-resolved absolute screenshot directory only when needed |
...
Canonical QA writes `latest.json`, `latest.md`, `latest.log`, cleanup provenance, and DPM seed
evidence under `output/front-office-qa/`.
...
- Use `-CleanCoreState` only for stale Core seed state, `-LotusAiEnvFile .env.example` for
  provider-disabled Advisor Brief proof, and `-RemoveImages` only when image freshness is required.
- Governed startup keeps `DEMO_DATA_PACK_ENABLED=false`. DPM writes use
  `lotus-platform.canonical-dpm-command-center-seed` with `manage.write`; use
  `Invoke-DpmCommandCenterSeed.ps1 -PreflightOnly` for a non-writing authority check.
- `DPM_CORE_CONTEXT_INCOMPLETE` means a Core source read was refused. Inspect the admitted request
  and response before assigning ownership; see `sgajbi/lotus-manage#711` and the historical
  `sgajbi/lotus-core#840` case.
- The wrapper preserves the Workbench-started `lotus-idea` runtime and verifies direct and ingress
  readiness. Use `Apply-DevIngressHosts-Elevated.ps1` when Windows blocks hosts-file updates,
  `-SeedWaitSeconds` for slow reseeds, and a caller-resolved absolute `-ScreenshotDirectory` when the
  default evidence path is unsuitable. Standalone `client-demo` leaves DPM seeding disabled.
