...
| Canonical front-office mainline certification | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -BringUp -RequireMainlineSources` | Fail closed unless every canonical participant is clean and exactly at `origin/main` before cleanup, Docker build, seed, validation, screenshots, and Lotus Idea readiness evidence; forces image builds for certified proof |
...
| Canonical orphan retirement | `python automation/canonical_orphan_retirement.py --help` | Dry-run or explicitly retire one exact container whose fresh cleanup plan proves a missing, unregistered labelled checkout; never removes projects, volumes, images, or networks |
| Canonical front-office clean rebuild | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -Clean -BringUp -BuildImages` | Clean only run-owned canonical Compose projects, rebuild images through the governed workbench runtime, and validate populated UI/product surfaces |
| Canonical all-app clean rebuild | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -Clean -BringUp -BuildImages` | Run the governed front-office clean rebuild, preserve the Workbench-seeded `lotus-idea` runtime, and probe direct host `8330` plus `idea.dev.lotus` ingress |
| Canonical front-office clean core reseed | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -BringUp -CleanCoreState -LotusAiEnvFile .env.example` | Reset only `lotus-core` Docker state before reseeding the governed front-office portfolio when stale core state blocks validation, with deterministic provider-disabled Advisor Brief proof |
| Canonical front-office clean core rebuild | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -BringUp -CleanCoreState -BuildImages -LotusAiEnvFile .env.example` | Reset `lotus-core` state, rebuild service images, reseed the governed portfolio, and validate populated UI/product surfaces with deterministic provider-disabled Advisor Brief proof |
| Canonical front-office full cleanup | `automation/Invoke-Canonical-FrontOffice-QA.ps1 -Clean -RemoveImages` | Remove containers, volumes, and local images owned by the planned canonical Compose projects without starting the stack |
| Dependency/container vulnerability exception register validation | `python automation/validate_vulnerability_exception_register.py --report-only` | Measure exception-register schema and semantic findings before dependency/security or release-image lane promotion; omit `--report-only` for focused blocking proof |
| Apply dev ingress hosts with elevation | `automation/Apply-DevIngressHosts-Elevated.ps1` | Launch the governed hosts sync with Administrator privileges and flush DNS when Windows blocks direct hosts-file writes |
...
Canonical front-office QA writes Docker cleanup scope, Compose-project ownership provenance, clean-core posture, mainline-source certification posture, Lotus AI env-file posture, canonical core demo-pack posture, DPM command-center seed status, ready/partial/empty command-center posture checks, seed wait, before/after artifact counts, and the runtime transcript path to `output/front-office-qa/latest.json` and `output/front-office-qa/latest.md`. `cleanup-plan-latest.json` is emitted before mutation; `-CleanPlanOnly` writes the same plan without deleting or starting anything. Cleanup requires both the exact canonical Compose project identity and exact normalized repository working directory; a nested worktree is not equivalent. The inventory explicitly maps `lotus-core`, repository-declared `lotus-core-app-local`, and isolated `lotus-core-canonical-ui` to the canonical `lotus-core` checkout boundary. It classifies conflicts as `active_foreign_owner`, `missing_labelled_checkout`, or `unproven_resource_only_owner` and does not infer aliases from name prefixes. Every conflict still blocks normal cleanup. An operator may use `canonical_orphan_retirement.py` only for a fresh, SHA-256-bound `missing_labelled_checkout` container after restating its exact ID, name, project, labelled path, and canonical root; the command rechecks path absence, registered Git worktrees, and live Docker labels immediately before the one-container action, writes a pre-mutation receipt, and records a regenerated remaining-conflicts view in the final receipt. Active, stale, changed, resource-only, volume, image, network, and broad project cleanup remains refused. Broad daemon-wide `lotus`, `pbwm`, or `performance` name-prefix cleanup is prohibited because it can destroy concurrent certification projects. The wrapper delegates all normal deletion to repository-scoped Workbench Compose teardown; that teardown removes the exact direct-ingress singleton before the Compose projects, and the platform wrapper then asserts that selected resources are gone. It also writes `output/front-office-qa/latest.log` and `output/front-office-qa/dpm-command-center-seed-latest.json` so operators can review seed readiness progression, source-product lineage, source-owned selection-basis evidence, source-backed DPM campaign definition/discovery evidence for `lotus-core:DpmPortfolioUniverseCandidate:v1`, canonical outcome-review create/list evidence, command-center supportability posture, current stateful action-register simulation evidence, Manage write-authorization preflight posture, response-body diagnostics, retry warnings, and teardown output after the terminal session is gone. Use `-RequireMainlineSources` for RFC closure, supported-feature promotion, or other mainline certification proof; it requires `-BringUp`, forces image builds, runs a fail-closed exact-`origin/main` source preflight before cleanup or startup, records `mainline_source_preflight`, and delegates the same Workbench provenance check into startup/validation. If this preflight fails, the wrapper does not stop an already-running canonical stack. Use `-CleanCoreState` only when `lotus-core` reseeding state is stale, `-LotusAiEnvFile .env.example` when Advisor Brief proof should avoid an unstarted live local model dependency, and `-RemoveImages` only when image freshness matters more than startup speed. Governed startup runs `lotus-core` with `DEMO_DATA_PACK_ENABLED=false` so the broad app-local demo pack is not part of canonical `PB_SG_GLOBAL_BAL_001` evidence. The DPM seed uses the governed local seed service identity `lotus-platform.canonical-dpm-command-center-seed` with `manage.write` capability; use `automation/Invoke-DpmCommandCenterSeed.ps1 -PreflightOnly` when diagnosing seed authority without writing DPM evidence. After preflight, `DPM_CORE_CONTEXT_INCOMPLETE` means a Core source request or readiness check was refused; inspect the outbound route, admitted header/body scope and source response before assigning ownership. The CoreSnapshot selector defect is `sgajbi/lotus-manage#711`; Core's earlier missing eligibility, tax-lot and market-data case is `sgajbi/lotus-core#840`. Governed startup also starts and seeds `lotus-idea`; the platform wrapper preserves that seeded runtime, then records direct `127.0.0.1:8330` and canonical `idea.dev.lotus` readiness checks without changing the governed Workbench panel-evidence contract. If the canonical hostname is missing and Windows denies hosts-file writes, run `automation/Apply-DevIngressHosts-Elevated.ps1`. Use `-SeedWaitSeconds <seconds>` for long canonical reseeds and `-ScreenshotDirectory <path>` when a demo pack must be written outside the default Workbench artifact folder. Use `-SkipDpmCommandCenterSeed` only for diagnostics that intentionally validate unseeded DPM empty/error posture.

The DPM seed resolves date-aligned portfolio cash evidence from Gateway before recalculating Manage
mandate health. It requests the Workbench overview without optional Performance or rebalance
enrichment so unrelated analytics or workflow availability cannot invalidate confirmed valuation
facts. Portfolio identity, business date, source warnings, partial failures, and
percentage-to-ratio normalization must all validate; missing or contradictory source evidence
still fails the seed instead of using a fallback.

## Dev Ingress Operator Loop

Use this sequence for RFC-0071 local ingress rollout:

1. Preview or apply the managed hosts block with `automation/Sync-Dev-Ingress-Hosts.ps1`; use `automation/Apply-DevIngressHosts-Elevated.ps1` when the apply needs Administrator privileges.
