...
def audit() -> list[Finding]:
    ...
    for skill_dir in skill_dirs():
        ...
        if manifest_entry is None:
            findings.append(
                Finding(skill_name, "high", "manifest-missing", "Skill folder is absent from lotus-skill-manifest.json.", str(skill_path))
            )

        extra_keys = sorted(set(frontmatter) - {"name", "description"})
        if extra_keys:
            findings.append(
                Finding(skill_name, "medium", "frontmatter-extra", f"Frontmatter has non-standard keys: {', '.join(extra_keys)}.", str(skill_path))
            )

        if frontmatter.get("name") != skill_name:
            findings.append(
                Finding(skill_name, "high", "frontmatter-name", "Frontmatter name does not match folder name.", str(skill_path))
            )

        description = frontmatter.get("description", "")
        if len(description) < 120:
            findings.append(
                Finding(skill_name, "medium", "description-short", "Description may be too short to route reliably.", str(skill_path))
            )
        if len(description) > 1024:
            findings.append(
                Finding(skill_name, "critical", "description-long", "Description exceeds 1024 characters.", str(skill_path))
            )
        if "use when" not in description.lower() and "apply when" not in description.lower():
            findings.append(
                Finding(skill_name, "medium", "description-trigger", "Description does not clearly include use/apply trigger wording.", str(skill_path))
            )

        line_count = skill_text.count("\n") + 1
        if line_count > 500:
            findings.append(
                Finding(skill_name, "medium", "skill-body-large", f"SKILL.md has {line_count} lines; consider moving details to references.", str(skill_path))
            )

        if "Continuous Skill Improvement" not in skill_text:
            findings.append(
                Finding(skill_name, "high", "continuous-improvement-missing", "Skill lacks Continuous Skill Improvement section.", str(skill_path))
            )
        ...
        if directly_owned and not agents_yaml.exists():
            findings.append(
                Finding(skill_name, "medium", "openai-metadata-missing", "Directly Lotus-owned skill lacks agents/openai.yaml.", str(agents_yaml))
            )
        if agents_yaml.exists():
            agent_text = read_text(agents_yaml)
            if "default_prompt:" not in agent_text or "short_description:" not in agent_text:
                findings.append(
                    Finding(skill_name, "medium", "openai-metadata-incomplete", "agents/openai.yaml is missing expected interface fields.", str(agents_yaml))
                )

        if directly_owned and skill_name not in routing_text:
            findings.append(
                Finding(skill_name, "medium", "routing-missing", "Directly Lotus-owned skill is not referenced in LOTUS-SKILL-ROUTING-MAP.md.", str(ROUTING_MAP_PATH))
            )
        ...
        for reference_path in sorted((skill_dir / "references").glob("*.md")):
            ...
            if reference_lines > 100 and not has_toc(reference_text):
                findings.append(
                    Finding(skill_name, "low", "reference-toc-missing", f"Reference has {reference_lines} lines without a contents section.", str(reference_path))
                )
            if reference_path.name not in skill_text:
                findings.append(
                    Finding(skill_name, "low", "reference-undiscoverable", "Reference file is not named from SKILL.md.", str(reference_path))
                )

        for script_path in sorted((skill_dir / "scripts").glob("*")):
            if script_path.is_file() and script_path.suffix in {".py", ".ps1"} and script_path.name not in skill_text:
                findings.append(
                    Finding(skill_name, "low", "script-undiscoverable", "Script is not named from SKILL.md.", str(script_path))
                )

    for manifest_name, entry in manifest.items():
        skill_path = PLATFORM_ROOT / str(entry.get("path", f"codex/skills/{manifest_name}")) / "SKILL.md"
        if not skill_path.exists():
            findings.append(
                Finding(manifest_name, "critical", "manifest-path-missing", "Manifest points to a missing skill path.", str(skill_path))
            )
