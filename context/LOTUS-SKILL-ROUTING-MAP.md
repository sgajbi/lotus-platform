...
These skills remain the right fit. RFC-0089, RFC-0090, and RFC-0091 add a more explicit routing row
for mesh certification, enterprise maturity checks, evidence-policy/lifecycle drift, and GitHub
cross-repo gate failures. No dedicated mesh-certification skill is created yet because the current
workflow is still well covered by backend delivery governance plus pre-merge governance; create one
only if repeated operational failure patterns prove that the current skill set is too broad.
