...
| Launch or monitor detached platform automation profiles, local background runs, or RFC-0095 heartbeat attention artifacts | `platform-automation-ops` | `async-task-runner`, `lotus-pr-premerge-gate`, `platform-pulse-monitor` | `context/playbooks/AGENT-CONTEXT-AND-TASK-LEDGER.md`, `platform-contracts/agent-engineering/engineering-task-ledger-contract.v1.json`, `platform-contracts/heartbeat/heartbeat-status.schema.json`, `automation/Start-Background-Run.ps1`, `automation/Check-Background-Runs.ps1`, `automation/Run-Heartbeat.ps1` |
