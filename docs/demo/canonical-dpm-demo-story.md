...
The canonical contract binds command-centre persistence and reads to the explicitly admitted
`tenant-sg` Workbench caller. Campaign and PM Operating Quality evidence are persisted and verified
for that same caller; there is no second query-tenant override, and caller-derived tenant authority
cannot be bypassed.
...
| Advisory bank-demo proof | RFC-0028 `advisory.bank_demo_proof` proof uses the governed `RFC28_BANK_DEMO_CLIENT_READY_PROOF_CANONICAL` scenario and Gateway `/api/v1/advisory/bank-demo-proof/*` routes. Workbench renders Advise-owned supported-claim classifications and blocked client-publication posture without local proof-pack construction, sign-off approval, client communication, orders, fills, settlement, or OMS claims. | `lotus-advise`, `lotus-gateway`, `lotus-workbench`, `lotus-platform` | `advisory-bank-demo-proof-live.png`, `BANK_DEMO_PROOF_PACK_CREATED` |
...
2. Run the platform canonical front-office QA wrapper:
   ...
   standalone recheck of an already-running stack; it excludes the presentation-backed Idea
   capacity probe and is not demo, capacity, or release evidence.
