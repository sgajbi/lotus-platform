...
| Advisory proposal and policy evidence | RFC-0023 advisor-review narrative, RFC-0024 advisor-use memo evidence, and RFC-0025 suitability policy review now use the governed `RFC23_25_ADVISORY_PROPOSAL_POLICY_CANONICAL` scenario. The policy leg creates a Singapore structured-note `PENDING_REVIEW` evaluation through Gateway and proves queue, workflow, sign-off package, blocked client-ready posture, and bounded request-more-evidence behavior before the Workbench Suitability Review screenshot is accepted. | `lotus-advise`, `lotus-gateway`, `lotus-workbench`, `lotus-platform` | `proposal-narrative-posture-live.png`, `proposal-memo-evidence-pack-live.png`, `advisory-suitability-review-live.png`, `POLICY_EVALUATION_PENDING_REVIEW_CREATED` |
