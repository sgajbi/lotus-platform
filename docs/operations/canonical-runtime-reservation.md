...
An expired live record cannot be silently removed. `reclaim` requires a new named holder, purpose, future expiry, the exact prior scope and handover receipt even if inventory is empty. The prior reservation is retained as `reclaimed`; events preserve the decision. The original holder may teardown an expired lease, but may not start another change. `recover` requires the same unexpired holder, preserves the exact acquired source/scope, and requires an explicit fresh handover receipt bound to the currently observed resources; it reconciles an interrupted operation or changed inventory even when repository checkouts have since advanced. Recovery does not authorize another change from the stale source vector: change preflight continues to compare current checkout sources and refuses until the old runtime is torn down and a fresh lease is acquired. Wrong/missing identities, ports, nested/foreign checkout paths, overlapping records, stale receipts and arbitrary finish tokens refuse without persisting a mutation.

Status, recovery, teardown and reclaim retain the acquired physical/source scope even when checkouts have advanced. They do not substitute new HEADs for old resource authority. A new change or proof still requires the current exact source vector; after releasing the old window, explicitly acquire the new one. This permits safe cleanup without labeling older images as the new source.
