...
- Scope:
  - `lotus-ai` RFC-0032
  - `lotus-ai` RFC-0033
  - `lotus-platform` RFC-0084
  - `lotus-platform` RFC-0085
...
| Platform-wide context truth | `lotus-platform/context/` and platform-owned docs | Governed by RFC-0073, RFC-0074, RFC-0084 |
...
| Background engineering task truth for GitHub checks | GitHub Actions and PR state | RFC-0085 records/aligns status but does not replace GitHub truth |
| Background engineering task truth for local automation | platform automation artifacts and scripts | RFC-0085 standardizes identity, lifecycle, and evidence posture |
...
| `engineering_task_id` | Durable identity for detached engineering work such as background runs or delegated subtasks | RFC-0085 |
| `task_kind` | Category of detached engineering work | RFC-0085 |
| `context assembly` | Governed selection of the smallest correct working set for a Lotus agent task | RFC-0084 |
| `compaction` | Governed reduction of session context while preserving exact engineering-critical identifiers and decisions | RFC-0084 |
| `promotion` | Moving durable session knowledge into governed artifacts such as docs, context files, skills, or validators | RFC-0084 |
