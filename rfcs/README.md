...
- `RFC-0104-batch-reporting-scheduler-concurrency-and-recovery.md` (in progress; Slice 0 platform scaffold guardrails, Slice 1 `lotus-report` batch-orchestrator module boundary, Slice 2 internal durable batch/item materialization, and Slice 3 deterministic schedule-cycle materialization are implemented, with scheduler loop, APIs, concurrency, retry, resume, progress, dispatch, and recovery runtime behavior still pending)
