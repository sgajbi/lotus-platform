...
- `RFC-0098-per-pack-queue-and-concurrency-policy.md` (partially implemented; first `lotus-ai` source-truth queue policy, admission, queue-status, queue-attention, durable queue-event history, terminal timeout/cancellation posture, retry/replay recovery-decision posture, repeated-failure cluster attention, and degraded queue-source attention waves are implemented; retry/replay execution and optional downstream publication remain future work)

## Recommended Next Implementation Order

1. `RFC-0098-per-pack-queue-and-concurrency-policy.md`
   Implement retry/replay execution or persisted queued-item lifecycle in `lotus-ai`; keep gateway and Workbench adoption deferred until a supported operator or product surface needs bounded queue posture.
