...
- `RFC-0095-heartbeat-driven-monitoring-and-attention-surfacing.md` (draft; recommended next implementation because it makes stale PRs, CI, local automation, workflow-pack posture, mesh evidence, wiki drift, and context drift visible before more long-running work expands)
- `RFC-0096-governed-multi-agent-delegation-model.md` (draft; implement after RFC-0095 so delegated work has heartbeat visibility and stale-task surfacing)
...
1. `RFC-0095-heartbeat-driven-monitoring-and-attention-surfacing.md`
   Establishes periodic, deduplicated attention surfacing before additional long-running work expands.
2. `RFC-0096-governed-multi-agent-delegation-model.md`
   Builds on RFC-0094 and RFC-0095 so parallel engineering work is bounded, observable, and evidence-bearing.
3. `RFC-0097-task-flow-runtime-for-long-running-workflow-packs.md`
   Adds multi-step workflow-pack runtime support after monitoring and delegation governance are in place.
4. `RFC-0098-per-pack-queue-and-concurrency-policy.md`
   Adds explicit per-pack queue, lane, timeout, and concurrency controls as task-flow runtime capacity needs become concrete.
