...
- `RFC-0109-lotus-production-principal-and-capability-resolution.md` (draft; resolves #563 and #775 into one principal design. The session half is already implemented as `platform-contracts/bff-principal-session/` and its validator passes; the downstream half is specified and unimplemented. States that a capability is resolved from a grant keyed by the verified principal rather than read from the request, gives Workbench #436, Gateway and Manage #624 independent acceptance that needs no identity provider, and separates the three genuinely external decisions: bank IdP selection, managed key custody, and production certification sign-off)
