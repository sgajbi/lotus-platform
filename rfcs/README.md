...
- `RFC-0098-per-pack-queue-and-concurrency-policy.md` (partially implemented; first `lotus-ai` source-truth queue policy, admission, queue-status, and queue-attention wave is merged and proven; durable queue-event history is under review in `sgajbi/lotus-ai#47`; optional downstream publication remains future work)

## Recommended Next Implementation Order

1. `RFC-0098-per-pack-queue-and-concurrency-policy.md`
   Land durable queue-event history and persisted queue lifecycle in `lotus-ai`; keep gateway and Workbench adoption deferred until a supported operator or product surface needs bounded queue posture.
