...
| `lotus_analytics_freshness_bucket_total` | domain services | `service`, `operation`, `freshness_bucket`, `supportability_state` |
...
`ai.observability.ai_surface_supportability`. lotus-ai PR #57 hardens that proof with bounded
`supportability_reason` values, explicit `metric_labels` truth, sensitive-diagnostic rejection
tests, Prometheus label-boundary tests, repo-native `make check`, `make ci`, Docker build, and
published wiki source. These proofs include repo-native tests, OpenAPI quality or contract proof
where applicable, docs, and wiki source updates. Remaining performance
...
lotus-performance PR #141 hardens explicit performance `metric_labels`, shared bounded Prometheus
label tuples, and no-sensitive metric-label proof for calculation supportability and backend
freshness metrics.
...
| `workbench.analytics.observability.entitlement_audit_events` | Implemented | `lotus-gateway` | Slice 7 adds Gateway product-safe selected analytics read audit logs. Successful upstream reads emit `analytics_read_allowed`; upstream `401`/`403` denials emit `analytics_read_denied`; tests prove bounded audit fields and exclusion of portfolio, client, request/response body, and raw entitlement-failure content. Slice 13 adds protected diagnostics lookup audit proof. Slice 19 keeps entitlement audit records bounded while promoting the current certified read-path caller-context certification to implemented. |
| `workbench.analytics.observability.caller_context_entitlement_certification` | Implemented | `lotus-gateway`, `lotus-workbench`, `lotus-platform` | Slice 19 certifies the current Workbench read paths end to end: Workbench PR #131 proves BFF caller-context forwarding defaults; Gateway PRs #174, #175, and #176 prove caller-context enforcement for performance summary, risk summary, and advisor-brief read/review-action routes; Gateway PRs #159 and #177 prove bounded allow/deny audit records; and Workbench PR #133 proves permission-blocked UI states for Summary, Risk Review, and Advisor Brief without raw entitlement response-body leakage. Future new certified read paths must add equivalent evidence before promotion. |
| `workbench.analytics.observability.advisor_brief_review_action_metrics` | Implemented | `lotus-workbench` | Workbench PR #134, merge `4d8bc144c8ce07cf13087c704deeec9f177987de`, implements bounded observability for the Advisor Brief review-action mutation. The proof covers the `performance-advisor-brief-review-action` observed surface, `performance.workspace.advisor-brief.review-action` operation, same-origin `/api/metrics/events` browser metric ingest, `/api/metrics` export, bounded `WorkbenchApiError` mutation failures without raw Gateway response bodies, exclusion of portfolio/client/advisor/correlation/free-form reason labels, local canonical Workbench BFF env hardening, live canonical validation for `PB_SG_GLOBAL_BAL_001`, Gateway log/trace inspection, green Workbench CI, and wiki publication `8bec78e`. |
| `workbench.analytics.observability.mutation_hydration_boundary` | Implemented | `lotus-workbench` | Workbench PR #136, merge `3dfdbd90878a82562ae38d8df66b1947ec16154f`, hardens the Workbench observation boundary so state-changing actions record API request and panel-state metrics without incrementing panel hydration. Live browser proof for `performance-advisor-brief-review-action` recorded `reviewActionStatus=200`, `reviewActionLineCount=11`, `hydrationReviewActionLineCount=0`, `hasApiRequestMetric=true`, `hasPanelStateMetric=true`, and `leakedForbidden=[]`; Gateway logs showed `ai.workflow-packs.runs.review-actions` with `state=ready` and `status_class=2xx`; Workbench wiki publication commit `2864f2c` records the durable operator rule. |
...
| `analytics.backend.observability.freshness_supportability` | Implemented | `lotus-performance`, `lotus-risk` | `lotus-performance` emits `calculation_supportability` and bounded `lotus_performance_calculation_supportability_total` labels for completed TWR, MWR, contribution, and attribution calculations through PR #138; PR #139 adds the generic RFC-0108 `lotus_analytics_freshness_bucket_total` companion metric from the same source-owned supportability recording path with bounded `service`, `operation`, `freshness_bucket`, and `supportability_state` labels. `lotus-risk` emits `metadata.calculation_supportability` and bounded `lotus_risk_calculation_supportability_total` labels for `risk/calculate`, drawdown, rolling metrics, historical attribution, and concentration through PR #107; PR #108 adds the same generic RFC-0108 companion metric. lotus-risk PR #109 hardens the source contract with explicit `metadata.calculation_supportability.metric_labels`, shared label tuples for `lotus_risk_calculation_supportability_total` and `lotus_analytics_freshness_bucket_total`, Prometheus exposition tests proving bounded labels only, no-sensitive label rejection for portfolio/client/correlation/trace/transaction/security/request/response identifiers, full local gates, green Feature Lane and PR Merge Gate, and published wiki source. Gateway PR #166 consumes those source supportability blocks through the experience API, maps stale/partial/unavailable posture into bounded fan-out state, and preserves source supportability in risk workspace supportability items plus performance evidence context. Workbench PR #118 reconciles Gateway `source_supportability` arrays into UI freshness/supportability derivation and canonical live validation summaries, and Workbench PR #119 plus lotus-core PR #328 prove clean governed canonical runtime execution. Full Workbench all-supported-surface promotion and full RFC-0079 risk/evidence scope remain planned until separately implemented and certified. |
| `core.observability.portfolio_supportability` | Implemented for portfolio readiness; Gateway and Workbench reconciliation implemented for supported portfolio readiness reads | `lotus-core`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `PortfolioReadinessResponse.supportability`, bounded `lotus_core_portfolio_supportability_total` labels, capability publication, repo-native unit/integration/OpenAPI/API-vocabulary/no-alias proof, docs, and wiki source updates. Gateway PR #167 preserves lotus-core portfolio readiness `supportability` through the portfolio readiness API and normalizes source freshness into Workbench vocabulary. Workbench PR #120 consumes the Gateway supportability block into the detailed portfolio workspace and records no-sensitive `portfolio.readiness` supportability metrics. lotus-core PR #329 hardens the source proof with explicit `metric_labels`, a shared observability label tuple, real Prometheus exposition tests proving only `state`, `reason`, and `freshness_bucket` labels, no-sensitive label rejection for portfolio/client/correlation/trace/security/request/response identifiers, full local `make ci-local`, `make security-audit`, and dividend-contract proof, green Feature Lane and PR Merge Gate including coverage, E2E smoke, latency, Docker smoke, and performance load gates, and published wiki source. |
...
| `risk.observability.calculation_supportability` | Implemented for risk endpoint families; Gateway and Workbench reconciliation implemented for supported risk reads | `lotus-risk` | Slice 12 partial proof implements `metadata.calculation_supportability`, bounded `lotus_risk_calculation_supportability_total` labels, integration capability publication, repo-native tests, OpenAPI quality proof, docs, and wiki source updates for `risk/calculate`. PR #107 completes the same source-owned supportability contract across drawdown, rolling metrics, historical attribution, and concentration with green Feature Lane and PR Merge Gate checks, docs, vocabulary, monetary-float guard baseline, and published wiki source. lotus-risk PR #109 hardens the source proof with explicit `metric_labels`, shared observability label tuples, real Prometheus exposition tests proving `lotus_risk_calculation_supportability_total` uses only `operation`, `supportability_state`, `reason`, and `freshness_bucket` and `lotus_analytics_freshness_bucket_total` uses only `service`, `operation`, `freshness_bucket`, and `supportability_state`, no-sensitive label rejection for portfolio/client/correlation/trace/transaction/security/request/response identifiers, full local lint/typecheck/OpenAPI/API-vocabulary/no-alias/domain-data/migration/unit/integration/e2e/coverage/security/Docker proof, green Feature Lane and PR Merge Gate, and published wiki source. Gateway PR #166 consumes risk source supportability into fan-out classification and risk workspace supportability items; Workbench PR #118 derives supported risk freshness/supportability from Gateway `source_supportability` and records live validation supportability checks. |
| `advise.observability.advisory_supportability` | Implemented for capability-level advisory supportability; explicit metric-label proof hardened; Gateway/Workbench reconciliation implemented for supported advisor-brief reads | `lotus-advise`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `GET /platform/capabilities` `supportability`, bounded `lotus_advise_advisory_supportability_total` labels, capability publication, repo-native `make check` proof, docs, and wiki source updates. lotus-advise PR #109 hardens explicit `supportability.metric_labels`, shared bounded labels for `lotus_advise_advisory_supportability_total`, OpenAPI/API-vocabulary proof, real Prometheus exposition tests proving only `state`, `reason`, and `freshness_bucket`, no-sensitive label rejection for portfolio/account/client/advisor/proposal/workspace/correlation/trace/request/transaction/security/request-body/response-body identifiers, local `make check` and `make ci-local` proof, green Feature Lane and PR Merge Gate including Docker build, and published wiki source. Gateway PR #172 preserves lotus-advise advisory supportability through advisor-brief responses. Workbench PR #125 consumes nested `advisory_supportability` for supported advisor-brief observability metrics without sensitive labels. |
...
| `render.observability.render_supportability` | Implemented for render metadata supportability; Gateway and Workbench reconciliation implemented for supported report-batch operator reads | `lotus-render`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `GET /metadata` `supportability`, bounded `lotus_render_supportability_total` labels, runtime/readiness/template-registry backed posture, repo-native `make check` proof, docs, and wiki source updates. PR #6 merged as `d2198f821516cb25bd8e0a36d42f10e04d7fdd5f` and wiki published as `34ecc05`. Gateway PR #171 adds render metadata consumption through report-batch create/status/run-once responses. Workbench PR #124 consumes nested `render_supportability` observability for report-batch operator reads. |
| `archive.observability.archive_supportability` | Implemented for archive metadata supportability; Gateway and Gateway-backed Workbench retrieval implemented | `lotus-archive`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `GET /metadata` `supportability`, bounded `lotus_archive_supportability_total` labels, retrieval/retention/legal-hold/access-audit/lifecycle backed posture, repo-native `make check` proof, docs, and wiki source updates. PR #17 merged as `031fcc1a2e46cc4750d47a06c99b65387e7b95ef` and wiki published as `98ce3b7`. Gateway archived document metadata/download routes exist. Workbench PR #126 adds report-batch operator archive metadata lookup and binary download links through `/api/bff/api/v1/documents/*`, preserves binary response headers, and publishes Workbench wiki source. lotus-archive commit `203ec6d` updates source metadata/docs/tests so `workbenchRetrievalSupported=true` means Gateway-backed Workbench retrieval only; direct Workbench-to-archive calls remain unsupported. |
| `ai.observability.ai_surface_supportability` | Implemented for AI surface runtime supportability; Gateway and Workbench reconciliation implemented for supported advisor-brief reads | `lotus-ai`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `observability_runtime.ai_surface_supportability` on `GET /platform/observability/runtime-status` and `GET /platform/runtime-status`, bounded `lotus_ai_surface_supportability_state` labels, workflow-pack runtime/provider/safety source posture for advisor brief, lotus-performance TWR inspection support brief, and workspace rationale surfaces, repo-native targeted tests/lint/typecheck proof, docs, and wiki source updates. lotus-ai PR #57 adds bounded `supportability_reason` values, explicit `metric_labels` truth for `surface`/`posture`/`source`, sensitive-diagnostic rejection tests, Prometheus label-boundary tests, `make check`, `make ci` with 99% coverage, Docker build proof, and published wiki source. Gateway PR #170 preserves lotus-ai AI surface supportability through advisor-brief responses. Workbench PR #123 consumes the Gateway supportability block for supported advisor-brief reads and records bounded no-sensitive supportability metrics. |
...
when any completed performance supportability surface is enabled, lotus-performance PR #141 proves
explicit performance `metric_labels` plus no-sensitive Prometheus label boundaries for performance
supportability and backend freshness metrics, and lotus-risk PR #109 proves explicit risk
`metric_labels` plus no-sensitive Prometheus label boundaries for risk supportability and backend
freshness metrics.
...
supported feature at that historical boundary. Later ecosystem slices promote Gateway fan-out
metrics and the performance/risk backend freshness metric; Workbench source-backed
freshness/degraded-state expansion remains planned for full RFC-0079 and all-surface coverage.
...
The final entitlement certification gap for current certified read paths is now closed by
`lotus-workbench` PR #133, merge `90a3d54e81a92e6b2bad8584edb25122cf3c2a81`, and Workbench wiki
publication commit `f523dbb`. The slice added bounded permission-blocked UI states for
performance Summary, Risk Review, and Advisor Brief; introduced shared Workbench API status
classification without reading or rendering raw Gateway entitlement bodies; fixed stale Advisor
Brief request handling; corrected a stale live-smoke layout assertion; and documented
permission-blocked analytics proof in the Workbench wiki. Local proof included full Workbench
unit/integration coverage (`151` files, `675` tests), focused permission-blocked tests, typecheck,
lint, `git diff --check`, canonical stack bring-up, `npm run live:validate`, platform canonical
QA evidence at
`lotus-platform/output/front-office-qa/canonical-front-office-qa-20260501-155832.md`, live
Workbench e2e smoke, Gateway/Performance log inspection for correlation/request/trace identifiers
and bounded `analytics_read_allowed` audit events, and clean runtime teardown. GitHub Feature Lane
and PR Merge Gate passed after a mock-contract fix, including Playwright smoke, Docker build, and
CI local Docker parity.

What was truly completed is now explicit in platform contracts: the certification contract marks
performance Summary, Risk Summary, and Advisor Brief read paths as implemented for current scope;
the observability supported-feature registry promotes
`workbench.analytics.observability.caller_context_entitlement_certification`; and validators require
Gateway allow/deny audit proof, Gateway caller-context enforcement, Workbench BFF caller-context
forwarding, and Workbench permission-blocked UI proof for each certified path. The remaining debt is
not hidden: future new certified read paths and full all-supported-surface promotion still require
their own implementation-backed slices, tests, live evidence, wiki publication, and clean PR
closure before they can be claimed.

Advisor Brief review-action observability closure:

`lotus-workbench` PR #134, merge `4d8bc144c8ce07cf13087c704deeec9f177987de`, closes the current
Advisor Brief review-action mutation observability gap. The slice added the
`performance-advisor-brief-review-action` observed surface and
`performance.workspace.advisor-brief.review-action` operation, wrapped the BFF review-action POST
in bounded Workbench API metrics, added same-origin browser metric ingest through
`/api/metrics/events`, and exported the accepted events through `/api/metrics`. The implementation
also bounded mutation failures through `WorkbenchApiError` without copying raw Gateway response
bodies, rejected forbidden metric labels such as portfolio, client, advisor, correlation, and
free-form reason values, and hardened local canonical Workbench startup so stale local BFF
environment values do not invalidate live proof.

What was proven is implementation-backed rather than cosmetic: focused Workbench tests, full
Workbench test/lint/typecheck gates, live canonical validation for `PB_SG_GLOBAL_BAL_001`,
browser-triggered review-action proof, `/api/metrics` export inspection, Gateway log/trace
inspection with correlation/request/trace identifiers, green Feature Lane and PR Merge Gate checks,
and Workbench wiki publication commit `8bec78e`. This promotes
`workbench.analytics.observability.advisor_brief_review_action_metrics` to implemented current
scope while preserving the broader all-supported-surface residual boundary.

Advisor Brief review-action mutation hydration boundary:

`lotus-workbench` PR #136, merge `3dfdbd90878a82562ae38d8df66b1947ec16154f`, closes a quality gap
found during live observability proof: state-changing actions were too easy to treat as panel
hydration events. The implementation adds a Workbench mutation-observation helper and applies it to
state-changing Workbench actions so mutations record bounded API request and panel-state metrics
without incrementing `lotus_workbench_panel_hydration_duration_seconds`.

What was proven is intentionally narrow and production-relevant: focused tests show the
`performance-advisor-brief-review-action` mutation emits API request and panel-state evidence,
does not emit panel hydration evidence, and excludes advisor identity plus free-form review reason
labels. Browser proof against the rebuilt canonical Workbench stack returned
`reviewActionStatus=200`, `reviewActionLineCount=11`, `hydrationReviewActionLineCount=0`,
`hasApiRequestMetric=true`, `hasPanelStateMetric=true`, and `leakedForbidden=[]`. Gateway logs
showed `ai.workflow-packs.runs.review-actions` completing with bounded correlation/request/trace
context, `state=ready`, and `status_class=2xx`. The platform contract now records the
`workbench-mutation-actions-exclude-panel-hydration` observation boundary, and validators reject
drift that would allow mutation evidence to be counted as panel hydration.

Core portfolio readiness metric-label hardening:

`lotus-core` PR #329, merge `f85de11c`, closes a source-supportability proof gap in the core
portfolio readiness contract. Portfolio readiness now publishes explicit `metric_labels` on the
supportability summary and uses a shared label tuple for
`lotus_core_portfolio_supportability_total`, so the API contract, implementation, OpenAPI
documentation, and Prometheus exposition stay aligned.

What was improved is production-relevant rather than cosmetic: the bounded metric-label contract is
centralized, unit tests inspect the real Prometheus exposition, integration tests assert the
serialized readiness response includes `metric_labels`, and OpenAPI tests assert the bounded label
contract is documented. The proof rejects portfolio, account, client, correlation, trace,
transaction, security, request-body, and response-body labels. Local evidence included focused
tests, 128 file-level operations/control-plane/OpenAPI tests, `make lint`, `make typecheck`,
`make openapi-gate`, `make ci-local`, `make security-audit`,
`make test-transaction-dividend-contract`, and `git diff --check`. GitHub Feature Lane and PR
Merge Gate passed, including coverage, E2E smoke, latency, Docker smoke, Docker build, and fast
performance load gates. Core wiki source was published as commit `b37d8d9`.

This promotes the quality of `core.observability.portfolio_supportability` for the current
implemented portfolio readiness scope while preserving the broader Workbench all-supported-surface
and RFC-0079 residual boundaries.

Risk supportability metric-label hardening:

`lotus-risk` PR #109, merge `b4efb74`, closes the same source-supportability proof gap for the risk
analytics supportability contract. Risk responses now publish explicit
`metadata.calculation_supportability.metric_labels`; `lotus_risk_calculation_supportability_total`
and `lotus_analytics_freshness_bucket_total` share bounded label tuples with the implementation and
documentation.

The proof is production-relevant: integration tests assert the serialized supportability response,
OpenAPI tests assert the label contract is documented, and Prometheus exposition tests assert the
two metric families expose only bounded labels while rejecting portfolio, account, client,
correlation, trace, transaction, security, request-body, and response-body label fields. Local
evidence included focused tests, `make lint`, `make typecheck`, `make openapi-gate`,
`make api-vocabulary-gate`, `make no-alias-gate`, `make domain-data-product-gate`,
`make migration-smoke`, `make test-unit`, `make test-integration`, `make test-e2e`,
`make test-pyramid-gate`, `make test-coverage`, `make security-audit`, `make docker-build`, and
`git diff --check`. GitHub Feature Lane and PR Merge Gate passed, including unit, integration,
e2e, coverage, test-pyramid, lint/typecheck/security, workflow lint, and Docker validation. Risk
wiki source was published as commit `3744871`.

This promotes the quality of `risk.observability.calculation_supportability` and
