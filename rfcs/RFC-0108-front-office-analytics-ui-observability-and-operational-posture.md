...
| `advise.observability.advisory_supportability` | Implemented for capability-level advisory supportability; Gateway/Workbench reconciliation implemented for supported advisor-brief reads | `lotus-advise`, `lotus-gateway`, `lotus-workbench` | Slice 12 partial proof implements `GET /platform/capabilities` `supportability`, bounded `lotus_advise_advisory_supportability_total` labels, capability publication, repo-native `make check` proof, docs, and wiki source updates. Gateway PR #172 preserves lotus-advise advisory supportability through advisor-brief responses. Workbench PR #125 consumes nested `advisory_supportability` for supported advisor-brief observability metrics without sensitive labels. |
