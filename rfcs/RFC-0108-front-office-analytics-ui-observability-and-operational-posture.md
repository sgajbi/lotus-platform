...
- Status: Reopened For Ecosystem Completion; Slice 13 Partial Gateway Diagnostics Proof Complete
- Date: 2026-04-29
...
and Slice 13 selected Gateway fan-out metrics plus protected diagnostics lookup are
implementation-backed. Backend freshness metrics, full caller-context entitlement certification,
all UI-facing Gateway fan-out rollout, and broad Workbench rollout remain planned until later
slices record code, tests, evidence, PRs, and merge state.
...
path. Gateway also exposes `GET /api/v1/analytics-ui/diagnostics/{support_reference}` for
protected operator diagnostics lookup, requires governed caller context plus an operator support
role, rejects raw-looking identifiers by requiring opaque `gdiag-*` support references, returns
only bounded support posture, and emits
...
rejection, OpenAPI visibility, and bounded audit fields. All-UI-facing fan-out path rollout remains
planned before Slice 13 can close.
...
| `gateway.analytics.observability.fanout_metrics` | Implemented for selected Workbench analytics fan-out; all-path rollout planned | `lotus-gateway` | Slice 13 partial proof emits `lotus_gateway_analytics_fanout_duration_seconds` and `lotus_gateway_analytics_degraded_total` from the selected Workbench performance and risk analytics fan-out path. Tests prove bounded labels, metric increments, and forbidden sensitive-label rejection. Dashboard reconciliation and all UI fan-out path coverage remain planned. |
...
| Ecosystem Slice 13: Gateway fan-out metrics, protected diagnostics, and audit completion | Partial `lotus-gateway` implementation for selected analytics fan-out metrics and protected diagnostics lookup; all UI-facing Gateway fan-out path rollout, OpenAPI certification for future changed APIs, and dashboard reconciliation remain planned | `lotus-gateway` PR #162: `make check`; `python -m pytest tests\unit\test_analytics_ui_observability_contract.py tests\unit\test_upstream_clients.py tests\integration\test_analytics_ui_diagnostics_api.py -q`; targeted `ruff`/`mypy`; migration smoke; Feature Lane and PR Merge Gate including Docker build/parity; wiki publication. `lotus-platform`: `python automation\validate_analytics_ui_observability_contract.py`; `python automation\validate_analytics_ui_ecosystem_completion.py`; targeted unit validators. | Partial: `gateway.analytics.observability.fanout_metrics` is promoted for selected Workbench performance and risk analytics fan-out, and `gateway.analytics.observability.protected_diagnostics` is promoted for `GET /api/v1/analytics-ui/diagnostics/{support_reference}` with `gdiag-*` opaque references, operator caller context, raw identifier rejection, bounded support posture, and safe `protected_diagnostics_lookup` audit fields. | Blocks ecosystem dashboards and final proof until all UI-facing Gateway fan-out paths are implemented and reconciled. |
