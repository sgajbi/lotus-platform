...
def test_rfc_0072_foundation_artifacts_are_present_and_cross_referenced() -> None:
    ...
    checklist = (ROOT / "rfcs" / "RFC-0072-implementation-checklist.md").read_text(
        encoding="utf-8"
    )
    mapping = (
        ROOT / "platform-standards" / "Repository-CI-Lane-Mapping-Baseline.md"
    ).read_text(encoding="utf-8")
    gap_audit = (
        ROOT / "platform-standards" / "Repository-CI-Convergence-Gap-Audit.md"
    ).read_text(encoding="utf-8")
    template_contract = (
        ROOT / "platform-standards" / "Backend-CI-Lane-Template-Contract.md"
    ).read_text(encoding="utf-8")
    hygiene_standard = (
        ROOT
        / "platform-standards"
    ...
    assert (
        "Slice 2 | Repository workflow classification and gap audit | Complete"
        in checklist
    )
    ...
    assert (
        "Slice 3C | Backend rollout wave 1 (`lotus-manage`, `lotus-report`) | Complete"
        in checklist
    )
    assert (
        "Slice 3D | Experience-layer rollout wave (`lotus-gateway`, `lotus-workbench`) | Complete"
        in checklist
    )
    assert (
        "Slice 3E | Analytics-domain rollout wave (`lotus-performance`, `lotus-risk`) | Complete"
        in checklist
    )
    assert (
        "Slice 3F | Shared capability rollout wave (`lotus-ai`) | Complete" in checklist
    )
    assert (
        "Slice 3G | Advisory-domain rollout wave (`lotus-advise`) | Complete"
        in checklist
    )
    ...
    assert (
        "Slice 4C | Repository governance policy normalization | Complete" in checklist
    )
    assert (
        "Slice 4D | Repository governance rollout and validation | Complete"
        in checklist
    )
    assert (
        "Slice 5A | Repository hygiene and dependency-model baseline | Complete"
        in checklist
    )
    assert (
        "Slice 5B | Workflow security and permissions baseline | Complete" in checklist
    )
    assert (
        "Slice 5C | Release evidence and SBOM scaffold baseline | Complete" in checklist
    )
    assert (
        "Slice 5D | Container build and image baseline enforcement | Complete"
        in checklist
    )
    assert (
        "Slice 4E | Platform validation coverage manifest and contract | Complete"
        in checklist
    )
    assert (
        "Slice 5E | Companion dependency lock scaffold baseline | Complete" in checklist
    )
    assert (
        "Slice 5F | Platform automation runtime isolation baseline | Complete"
        in checklist
    )
    assert (
        "Slice 5G | Repository text and line-ending hygiene scaffold baseline | Complete"
        in checklist
    )
    assert "Slice 5H | Workflow action runtime baseline | Complete" in checklist
    assert (
        "Slice 5I | Implementation-state reconciliation and governance drift cleanup | Complete"
        in checklist
    )
    ...
    assert (
        "workbench PR #77 was updated from `main`, conflict-resolved, and returned to green"
        in checklist
    )
    assert "There are no RFC-0072 implementation deviations" in checklist
    assert (
        "required approving review count is `0` for the single-developer operating model"
        in checklist
    )
    assert "Current scaffold source of truth" in checklist

    assert (
        "| `lotus-core` | Domain API | `.github/workflows/feature-lane.yml`, `.github/workflows/pr-merge-gate.yml`, `.github/workflows/main-releasability.yml`, `.github/workflows/pr-auto-merge.yml`"
        in mapping
    )
    ...
    assert (
        "Platform validation is normalized and must keep expanding with product coverage"
        in gap_audit
    )
    assert (
    ...
    assert (
        "Explicit `feature-lane.yml`, `pr-merge-gate.yml`, `main-releasability.yml`, and `platform-end-to-end-validation.yml` now exist"
        in gap_audit
    )
    assert "platform-end-to-end-validation.yml" in gap_audit
    assert (
        "Strong explicit lane split while retaining heavy gates for load, latency, Docker smoke, coverage, and institutional sign-off evidence"
        in gap_audit
    )
    assert (
        "branch protection is governed through `repository-governance-policy.json`"
        in gap_audit
    )
    ...
    assert (
        ...
        in auto_merge_template
    )
    assert "exit 0" in auto_merge_template
...
def test_platform_standards_and_runbook_point_to_rfc_0072_sources() -> None:
    ...
    procedural_memory_index = (
        ROOT / "context" / "PROCEDURAL-MEMORY-INDEX.md"
    ).read_text(encoding="utf-8")
    context_reference_map = (ROOT / "context" / "CONTEXT-REFERENCE-MAP.md").read_text(
        encoding="utf-8"
    )
    workflow_standard = (
        ROOT / "platform-standards" / "Development-Workflow-and-CI-Strategy-Standard.md"
    ).read_text(encoding="utf-8")
    local_runbook = (
    ...
    assert (
        "merged-to-main and validated as the definition of done"
        in bank_buyable_contract
    )
    assert "Test-family breadth is measured" in bank_buyable_contract
    assert (
        "Report-only quality inventories should be promoted to regression-blocking gates"
        in bank_buyable_contract
    )
    assert (
        "When one scanner supports both report generation and blocking enforcement"
        in bank_buyable_contract
    )
    assert "agentic coding quality evaluation loop" in bank_buyable_contract
    assert (
        "Complete one implementation-backed slice before opening the next"
        in bank_buyable_contract
    )
    assert "Separate design modularity from runtime modularity" in bank_buyable_contract
    assert (
        "AI, ML, or RAG assistance is advisory unless deterministic evidence"
        in bank_buyable_contract
    )
    assert "repository-approved linear merge path" in bank_buyable_contract
    assert (
        "test-family breadth, including API/runtime, contract/governance"
        in backend_refactor_instructions
    )
    assert (
        "When promoting a report-only inventory, use the `lotus-ci-enforcement-governance` intake"
        in backend_refactor_instructions
    )
    ...
    assert (
        "Design modularity is required before runtime modularity"
        in backend_refactor_instructions
    )
    assert (
        "API and error-model polish is part of the refactor"
        in backend_refactor_instructions
    )
    assert "exact blocker codes" in backend_refactor_instructions
    assert (
        "Live API tests are valuable when upstreams are available"
        in backend_refactor_instructions
    )
    ...
    assert (
        "docs/standards/Continuous Integration, Validation, and Release Governance Standard.md"
        in workflow_standard
    )
    assert (
        "`Continuous Integration, Validation, and Release Governance Standard.md`"
        not in workflow_standard
    )
...
def test_backend_lane_templates_exist_and_define_explicit_lane_names() -> None:
    ...
    feature_lane = (workflows_dir / "feature-lane.backend.template.yml").read_text(
        encoding="utf-8"
    )
    pr_merge_gate = (workflows_dir / "pr-merge-gate.backend.template.yml").read_text(
        encoding="utf-8"
    )
    main_releasability = (
        workflows_dir / "main-releasability.backend.template.yml"
    ).read_text(encoding="utf-8")
    standards_validator = (
        ROOT / "automation" / "Validate-Backend-Standards.ps1"
    ).read_text(encoding="utf-8")
...
def test_platform_repo_lane_workflows_and_shared_entrypoint_exist() -> None:
    feature_lane = (ROOT / ".github" / "workflows" / "feature-lane.yml").read_text(
        encoding="utf-8"
    )
    pr_merge_gate = (ROOT / ".github" / "workflows" / "pr-merge-gate.yml").read_text(
        encoding="utf-8"
    )
    main_releasability = (
...
def test_platform_validation_lane_workflow_and_shared_entrypoint_exist() -> None:
    ...
    validation_entrypoint = (
        ROOT / "automation" / "Invoke-PlatformValidationLane.ps1"
    ).read_text(encoding="utf-8")
...
def test_backend_governance_policy_tracks_wave_one_repo_lane_names() -> None:
    policy = (ROOT / "automation" / "repository-governance-policy.json").read_text(
        encoding="utf-8"
    )
    governance_enforcer = (
        ROOT / "automation" / "Enforce-Repository-Governance.ps1"
    ).read_text(encoding="utf-8")
    governance_validator = (
        ROOT / "automation" / "validate_repository_governance.py"
    ).read_text(encoding="utf-8")
    hygiene_validator = (
        ROOT / "automation" / "validate_repository_hygiene.py"
    ).read_text(encoding="utf-8")
    workflow_validator = (
        ROOT / "automation" / "validate_workflow_security.py"
    ).read_text(encoding="utf-8")
    workflow_action_validator = (
        ROOT / "automation" / "validate_workflow_action_runtime.py"
    ).read_text(encoding="utf-8")
    ...
    assert (
        "PR Merge Gate / Tests (transaction-portfolio-flow-bundle-contract)" in policy
    )
    ...
    assert "repository-governance-validation" in (
        ROOT / "automation" / "task-profiles.json"
    ).read_text(encoding="utf-8")
