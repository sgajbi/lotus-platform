...
8. review `output/front-office-qa/cleanup-plan-latest.json` before canonical cleanup; require exact
   ...
   exact checkout provenance, must block cleanup before mutation. A governed image may prove the
   checkout through its immutable `com.lotus.repository.checkout` label; the value must exactly
   equal the registered repository root. Residual volumes still require live-container proof. The cleanup inventory
